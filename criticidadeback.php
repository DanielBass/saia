          ...
          <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu">
                  ...
                  <a class="nav-link  dropdown-item text-primary" data-toggle="modal" data-target="#modalFoto" data-whatever="@pro" href="">Relátorio de imagens</a>
                </div>
            </li>
...
<div class="modal fade" id="modalRelatorio01" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Informação parar Gerar o Relatório de impactos</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<?php
    ...
    echo "<div class='textoreduzido'><p class='text-center'>Na Matriz Turbinada de Leopold essa criticidade, que é baseada em critérios agroecológicos, é medida em forma de cores. Valores numéricos (pesos) são associados aos parâmetros de todos os critérios, de acordo com o seu grau de influência nas condições ambientais de foco agroecológico. Então, um item de maior impacto agroecológico tem peso 2 (dois), um item de impacto médio, tem peso 1 (um) e um item com pouco impacto aos preceitos da agroecologia, tem peso zero.  
Esses valores são somados e as células, de impactos negativos cuja soma atingisse mais de 2/3 do valor máximo possível, são pintadas de vermelho e os positivos de verde. Aquelas, cuja soma dos valores não atingissem 1/3 do valor máximo possível ficariam em branco, as demais, sendo negativas são coloridas com amarelo e da cor ciano as que forem positivas</p></div>" ;
    
    function  calculando($inicial,$nomeAtividade,$nomeEtapa,$itemm){
      ...
      if($c1=='N'){
         ...
         if($total>15){
           ...
           $GLOBALS['altaNegativa']=$GLOBALS['altaNegativa']."<br>";
           $GLOBALS['altaNegativa'] =$GLOBALS['altaNegativa']."item: <b>".$itemm."</b> da ativdade <b>".$nomeAtividade."</b> da etapa<b> ".$nomeEtapa."</b>"; 
           $GLOBALS['altaNegativa']=$GLOBALS['altaNegativa']."<br>"; 

         }else if($total> 7 && $total<= 15){

           $GLOBALS['mediaNegativa']=$GLOBALS['mediaNegativa']."<br>";
           $GLOBALS['mediaNegativa'] =$GLOBALS['mediaNegativa']."item: <b>".$itemm."</b> da ativdade<b> ".$nomeAtividade."</b> da etapa <b>".$nomeEtapa."</b>"; 
           $GLOBALS['mediaNegativa']=$GLOBALS['mediaNegativa']."<br>";
                        
         }else{

           $GLOBALS['baixa']=$GLOBALS['baixa']."<br>";
           $GLOBALS['baixa'] =$GLOBALS['baixa']."item: <b>".$itemm."</b> da ativdade <b>".$nomeAtividade."</b> da etapa<b> ".$nomeEtapa."</b>"; 
           $GLOBALS['baixa']=$GLOBALS['baixa']."<br>";
         } 
         ...
         if ($total>15){
           ...
           $GLOBALS['altaPositiva']=$GLOBALS['altaPositiva']."<br>";
           $GLOBALS['altaPositiva'] =$GLOBALS['altaPositiva']."item: <b>".$itemm."</b> da ativdade <b>".$nomeAtividade."</b> da etapa <b>".$nomeEtapa."</b>"; 
           $GLOBALS['altaPositiva']=$GLOBALS['altaPositiva']."<br>";            

         }else if($total > 7 && $total<= 15){
            
           $GLOBALS['mediaPositva']=$GLOBALS['mediaPositva']."<br>";
           $GLOBALS['mediaPositva'] =$GLOBALS['mediaPositva']."item: <b>".$itemm."</b> da ativdade<b> ".$nomeAtividade."</b> da etapa <b>".$nomeEtapa."</b>"; 
           $GLOBALS['mediaPositva']=$GLOBALS['mediaPositva']."<br>";            
    ...
    if(isset($_SESSION ['idUsuario']) && empty($_SESSION ['idUsuario'])==false ){
        ...
        $idUsuario=$_SESSION ['idUsuario'];
        $idProcesso= $_POST['nomeProcessoC'];
        ...
        if($sql->rowCount()>0){
            ...
            foreach ($sql->fetchAll() as $item1){
                ...
                $oii = $item1['id_atividade'];

                $sql3=$db->query("SELECT nomeEtapa FROM etapas WHERE id_etapa = '$oi'");
                 $x=$sql3->fetch();
                 $nomeEtapa=$x['nomeEtapa'];

                $sql2=$db->query("SELECT * FROM itens WHERE id_atividade = '$oii'");
                ...
                foreach($sql2->fetchAll() as $item2){
                    ...
                    $itemm = $item2['item']; 
                    calculando($iniciais,$nomeAtividade,$nomeEtapa,$itemm);
         ...
         echo"<div class='textoreduzido text-center'>";   
         echo "<br><b>Negativo Altos</b> </br>";
         echo $altaNegativa;

         echo "<br><b>Negativo Médio</b> </br>";
         echo $mediaNegativa;

         echo "<br><b> Positivo Alto</b></br>";
         echo $altaPositiva;

         echo "<br><b> Média Positivo</b></br>";
         echo $mediaPositva;

         echo "<br><b> Baixa</b> </br>";
         echo $baixa;
         echo"</div>";
